<template>
    <hr-simple-select ref="select" v-slot="props" :class="[css_root]" v-bind="core.props" v-on="core.listeners"
        ...
        <h-floating as="div" :class="[css_ec('menu-container')]" :reference="button"
            ...
            <div :class="[css_ec('menu')]" @keydown="props.onKeyEvents" :style="{maxHeight: menuMaxHeight}">
                <h-scroll-container>
                    <div v-for="item in props.items" :class="[css_ec('item')]" @click="props.onItemClick(item)">
                        <slot :item="item">
                            <h-icon v-if="item.icon" :class="[css_ec('item-icon')]" :icon="item.icon" size="16" />
                            <span :class="[css_ec('item-label')]">{{ item.label }}</span>
                        </slot>
                        <div :class="[css_ec('item-checkmark'), {'-selected': item.selected}]">
                            <h-icon :icon="Icons.check" size="16px" />
                        </div>
                    </div>
                </h-scroll-container>
            </div>
...
export default defineComponent({
    ...
    setup(props, ctx) {
        ...
        const onFocusedItemChanged = (item: CoreSimpleSelectItemInstance) => {
        ...
        const asProps = (slotProps: CoreSimpleSelectSlotProps) => ({
            class: { '-active': slotProps.menuVisible, '-disabled': slotProps.disabled },
        });
        const core = coreSimpleSelectSetup().as('div', asProps).props(['modelValue', 'items', 'disabled', 'hideOnSelect', 'allowClear', 'multiple', 'compare']).events(['update:modelValue', 'focusedItemChanged'])
            .build();
