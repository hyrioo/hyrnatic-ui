export const previewExample = (options: {splitButton: boolean, hideOnClick: boolean, align: string}) => {
    const code =
`<h-dropdown label="Action" align="${options.align}" @click="onClick('Action')"${options.splitButton?' split-button':''}${options.hideOnClick?' hide-on-click':''}>
